...
export default function CvViewer({ fileId, filename, mimeType, height = "70vh", className = "" }) {
  ...
  if (!fileId) {
    return (
      <div className="rounded-tile border border-ink-600 bg-ink-850 p-6 text-center text-sm text-mist-500">
  ...
  return (
    <div className={cx("space-y-3", className)}>
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-tile border border-ink-600 bg-ink-850 px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-tile bg-ink-800 text-mist-400">
        ...
        <button
          type="button"
          onClick={download}
          className="inline-flex shrink-0 items-center gap-2 rounded-tile border border-ink-600 bg-ink-800 px-3 py-2 text-xs font-semibold text-mist-200 hover:bg-ink-700"
        >
      ...
      {loading && (
        <div
          className="flex items-center justify-center rounded-tile border border-ink-600 bg-ink-800"
          style={{ height }}
        >
          <div className="flex flex-col items-center gap-2 text-sm text-mist-500">
            <Loader2 className="h-6 w-6 animate-spin text-brand-hi" aria-hidden="true" />
            Loading CV…
          </div>
        </div>
      )}
      ...
      {error && !loading && (
        <div className="flex items-start gap-2 rounded-tile border border-risk/35 bg-risk/12 p-4">
      ...
      {!loading && !error && objectUrl && (
        <iframe
          ...
          className="w-full rounded-tile border border-ink-600 bg-ink-800"
          style={{ height }}
        />
      )}

      {!loading && !error && !objectUrl && text !== null && (
        <div className="rounded-tile border border-ink-600 bg-ink-800 p-4" style={{ maxHeight: height, overflowY: "auto" }}>
          <div className="mb-3 rounded-tile border border-raw/35 bg-raw/12 px-3 py-2 text-xs text-raw">
          ...
          {text ? (
            <pre className="whitespace-pre-wrap break-words font-sans text-sm leading-6 text-mist-200">{text}</pre>
          ) : (
            <div className="text-sm text-mist-500">No text could be extracted from this file.</div>
          )}
        </div>
      )}
    </div>
